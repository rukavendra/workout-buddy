...
const WorkoutDetails = ({workout}) => {
  ...
  const handleDelete= async()=>{
    ...
    const response = await fetch(`/workouts/${workout._id}`,{ method:"DELETE",headers: {'Authorization': `Bearer ${user.token}`}})
  ...
  const handleUpdate= async()=>{
    const response= await fetch('/workouts'+workout._id, { method: 'PATCH'})
    const json = await response.json()
    if(response.ok){
